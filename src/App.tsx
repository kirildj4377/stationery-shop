    ...
    >
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6 p-4">
        {filtered.slice(0, visibleCount).map((item, idx) => {
          ...
          return (
            <div key={idx} className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-xl transition-shadow">
              ...
              <h2 className="font-bold text-gray-800 text-sm mb-3 line-clamp-2 h-10">{title}</h2>
              <div className="flex flex-col gap-3">
                <span className="text-xl font-black text-blue-600">{price} грн</span>
                <button 
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex justify-end backdrop-blur-sm">
          <div className="bg-white w-full max-w-md h-full p-6 shadow-2xl flex flex-col">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsCartOpen(false)} className="text-3xl hover:text-red-500">&times;</button>
            </div>
            
            <div className="flex-1 overflow-y-auto pr-2">
              {cart.length === 0 ? <p className="text-gray-400 text-center mt-10">Кошик порожній...</p> : 
                cart.map((item, i) => (
                  <div key={i} className="flex justify-between items-center mb-4 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <span className="text-sm font-semibold flex-1">{item.title || item.Название || item.Найменування}</span>
                    <div className="flex items-center gap-4 ml-4">
                      <span className="font-bold text-blue-600 whitespace-nowrap">{item.price || item.Цена || item.Ціна} грн</span>
            ...
            {cart.length > 0 && (
              <div className="border-t pt-6 mt-4">
                <div className="mb-4">
                  <label className="block text-sm font-bold text-gray-700 mb-2 text-left">Номер телефону:</label>
                  <input 
                    type="tel"
                    placeholder="+380"
                    value={phone}
                ...
                <div className="flex justify-between text-2xl font-black mb-6 px-1">
                  <span>Разом:</span>
                  <span className="text-blue-600">{cart.reduce((sum, item) => sum + Number(item.price || item.Цена || item.Ціна || 0), 0)} грн</span>
                </div>
                <button 
                  onClick={sendOrder}
                  className="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl text-lg shadow-lg hover:bg-blue-700 transition-all"
                >
                  Оформити замовлення
                </button>
              </div>
